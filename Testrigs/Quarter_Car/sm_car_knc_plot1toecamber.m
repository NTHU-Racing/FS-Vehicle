function [TSuspMetrics, toeCurve, camCurve, KNCRes, pxCurve,pyCurve,strCurve, fzCurve, vrtCurve] = sm_car_knc_plot1toecamber(logsout,AxNum,showplot,calcMetrics,debugPlots)
%sm_car_knc_plot1toecamber  Obtain toe curve, camber curve, and suspension metrics
%   [TSuspMetrics, toeCurve, camCurve] = sm_car_knc_plot1toecamber(logsout,AxNum,showplot,calcMetrics,debugPlots)
...
%       toeCurve     Data for toe curve (toe angle vs. pz wheel center)
%       camCurve     Data for camber curve (camber angle vs. pz wheel center)
%       KNCRes       Structure with KnC results grouped by test phase
%       pxCurve      Lookup table, px wheel center       vs pz wheel center
%       pyCurve      Lookup table, py wheel center       vs pz wheel center
%       strCurve     Lookup table, toe angle             vs hand wheel angle
...
if(calcMetrics)
    ...
    % KnC model offsets frame so that contact patch is at z=0
    % Wheel center will start with z = tire_radius
    [tire_radius, ~] = sm_car_get_TireRadius(Vehicle);
    simlog_zWChp = tire_radius(AxNum);
...
if(calcMetrics)
    [TSuspMetrics,KNCRes] = sm_car_knc_calc_susp_metrics(...
        simlog_aToe, ...
        simlog_aCamber, ...
        simlog_aToeX,...
        simlog_aCamberX,...
